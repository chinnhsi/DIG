    ...
    <div class="label-core" tabindex="1">
      <div class="label-toolbar">
        <ul>
          ...
          <div class="label-divider"></div>
          <a :style="{ display: account.group == 3 ? 'block' : 'none' }" @click="showDelete" class="delete" title="删除" href="javascript:void(0)">
            <li>
      ...
      <div class="label-area">
        <div class="label-main">
          <img @load="() => loadMark()" :src="img" alt srcset />
      ...
      <div class="label-list">
        ...
        <ul v-if="type != 6" class="label-list-ul"></ul>
        <div v-else class="label-attr-wrapper">
          <h1>图片属性选择</h1>
          <div class="label-attr-type">
            <el-radio v-for="(val, key) in attr" :key="key" v-model="attrSelect" :label="key">{{ key }}</el-radio>
          </div>
          <div class="label-attr-list" v-if="attrSelect && attr[attrSelect]">
            <div class="label-attr-block" v-for="(val, key) in attr[attrSelect]" :key="key">
              <template v-if="!val.value">
                <h2>{{ key }}</h2>
                <div v-for="(v, k) in attr[attrSelect][key]" :key="k" class="label-attr-item">
                  <h3>{{ k }}</h3>
                  <el-select
                    v-model="attrValue[attrSelect][key][k]"
                    collapse-tags
                    size="small"
                    multiple
                    :multiple-limit="v.type == 'single label' ? 1 : 0"
                    placeholder="请选择"
                  >
                    <el-option v-for="item in v.value" :key="item" :label="item" :value="item"> </el-option>
                  </el-select>
                </div>
              </template>
              <template v-else>
                <div class="label-attr-item">
                  <h3>{{ key }}</h3>
                  <el-select
                    v-model="attrValue[attrSelect][key]"
                    size="small"
                    multiple
                    :multiple-limit="val.type == 'single label' ? 1 : 0"
                    placeholder="请选择"
                    collapse-tags
                  >
                    <el-option v-for="item in val.value" :key="item" :label="item" :value="item"> </el-option>
                  </el-select>
                </div>
              </template>
            </div>
          </div>
          <!-- <el-button @click="saveAttrLabel" size="small" style="width: 100%; margin-top: 10px;" type="primary">保存</el-button> -->
        </div>
...
<script>
import { getImg, getPendingUserList, setFinalVersion, saveLabel, deleteImageById, saveAttrLabel } from "@/models/Task.js";
...
export default {
  ...
  methods: {
      ...
      getImg(this.id, this.account.group == 1 ? this.account.id : this.penddingUserSelectId, this.account)
        .then((data) => {
          ...
          if (this.type != 6) {
            ...
            // let d = TestData;
            let d = data.data.labels;
            let t = {};

            let recur = function (source, target) {
              for (const key in source) {
                if (source.hasOwnProperty(key)) {
                  ...
                  target[key] = {};
                  let r = recur(element, target[key]);
                  if (!r) {
                    target[key] = [];
                  }
                }
            ...
            recur(d, t);
            console.log(t);
            if (Object.keys(data.data.datas) > 0) {
              t = Object.assign(t, data.data.datas);
            }
            this.attr = d;
            this.attrValue = t;
    ...
    saveLabelArea(data) {
      ...
      let ps = saveLabel(this.id, postData, this.account.group == 1 ? this.account.id : this.penddingUserSelectId);
      ps.then(() => {
    ...
    saveAttrLabel() {
      if (this.attrSelect === null) {
        this.$message({ type: "error", message: `保存失败，请选择完整(-1)` });
        return;
      }
      if (JSON.stringify(this.attrValue[this.attrSelect]).indexOf("[]") !== -1) {
        this.$message({ type: "error", message: `保存失败，请选择完整(-2)` });
        return;
      }
    ...
    next() {
      ...
      if (task.list.length - 1 == task.index) {
        ...
        getImgList(this.task.id, this.task.page).then((data) => {
          let list = data.data.images;
          this.task.list = list;
          console.log("success");
          console.log(task.list[task.index].imageId);
...
.label-attr-wrapper h1 {
  font-size: 18px;
  margin: 2px 0;
}
.label-attr-wrapper h2 {
  font-size: 16px;
  margin: 5px 0 10px;
}
.label-attr-item {
