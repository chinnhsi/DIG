<template>
  <div>
    ...
    <el-table border :data="taskData.list" stripe style="width: 100%; margin-top: 10px;" v-loading="loading">
      ...
      <el-table-column fixed="right" label="操作" width="530">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.taskType != 6" @click="editLabel(scope.row)" size="mini">选择标签</el-button>
    ...
    >
      <el-table
        ref="table"
        :data="userSelect.tableData"
        style="width: 100%;"
        @selection-change="handleSelectionChange"
        v-loading="userSelect.loading"
      >
    ...
    <el-dialog title="任务类型" :visible.sync="typeSelect.dialogVisible" width="30%" v-loading="typeSelect.loading">
      <el-form :inline="true">
        ...
        <el-form-item v-if="typeSelect.select == 6" label="文件" prop="file">
          <input @change="selectFile" type="file" accept="application/json" />
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
    ...
    <el-dialog title="导入新任务" :visible.sync="newTask.dialogVisible" width="60%" v-loading="newTask.loading">
      <el-table border :data="newTask.list" stripe style="width: 100%; margin-top: 10px;" v-loading="loading">
        <el-table-column prop="taskName" min-width="100" label="任务名"></el-table-column>
        <el-table-column label="设置类型">
          <template slot-scope="scope">
            <template v-if="scope.row.taskType != 5">
...
export default {
  ...
  methods: {
          ...
          reader.onloadend = function (evt) {
            if (evt.target.readyState == FileReader.DONE) {
              ...
              try {
                ...
                this.$message({ type: "error", message: `选择文件内容解析错误，请重新选择` });
                return;
              }
              console.log(j);
              cb(JSON.stringify(j));
    ...
    typeSubmit() {
      if (this.typeSelect.select == 6 && this.file == null) {
