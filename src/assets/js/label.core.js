...
function LabelCore(
  ...
  const event = {
    ...
    mouseup: function(e) {
      ...
      if (mode == 3) {
        ...
        if (!nowDrawType) {
          let middleX =
              (pointList[selectPoint][0] +
                pointList[
                  selectPoint - 1 < 0 ? pointList.length - 1 : selectPoint - 1
                ][0]) /
              2,
            middleY =
              (pointList[selectPoint][1] +
                pointList[
                  selectPoint - 1 < 0 ? pointList.length - 1 : selectPoint - 1
                ][1]) /
              2;
          ...
          middleX =
            (pointList[selectPoint + 1][0] +
              pointList[
                selectPoint + 2 > pointList.length - 1 ? 0 : selectPoint + 2
              ][0]) /
            2;
          middleY =
            (pointList[selectPoint + 1][1] +
              pointList[
                selectPoint + 2 > pointList.length - 1 ? 0 : selectPoint + 2
              ][1]) /
            2;
  ...
  const buttonMethod = {
    ...
    dragPoint: function(x, y, clientX, clientY) {
      ...
      if (tempSelectPoint >= 0 && pointList[tempSelectPoint][2] == 1) {
        if (
          selectPoint != tempSelectPoint &&
          selectPoint != tempSelectPoint + 2
        ) {
          if (selectPoint != -1) {
            selectPoint =
              selectPoint > tempSelectPoint + 1
                ? selectPoint - 2
                : selectPoint == tempSelectPoint + 1
                ? tempSelectPoint
                : selectPoint;
  ...
  const method = {
    ...
    drawRectangle: function(x, y) {
      ...
      pointList[pointIndex + 1 > 3 ? 0 : pointIndex + 1][pos1] =
        pos1 == 0 ? x : y;
      pointList[pointIndex][0] = x;
      pointList[pointIndex][1] = y;
      pointList[pointIndex - 1 < 0 ? 3 : pointIndex - 1][pos1 == 0 ? 1 : 0] =
        pos1 == 0 ? y : x;
  ...
  let init = function() {
    ...
    console.log(
      Math.abs(area.clientHeight * scale - height) +
        "  " +
        Math.abs(area.clientWidth - width)
    );
    scaleCanvas = 1;
    // Math.abs(area.clientHeight * scale - height) >
    // Math.abs(area.clientWidth - width)
    ...
    if (labelList) {
      ...
      for (const item of temp) {
        item.addEventListener("click", function() {
          this.visible =
            this.visible || this.visible == undefined ? false : true;
          ...
          this.classList.remove(
            !this.visible ? "icon-kejian" : "icon-bukejian"
          );
