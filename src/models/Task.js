...
function setFinalVersion(iid, uid) {
  let group = localStorage.getItem("group");
  let d = getUserGroupStr(group);
  return ajaxPost(`/api/${d}/setFinalVersion`, {
    imageId: iid,
    userConfirmId: uid
  });
}
...
function saveAttrLabel(iid, data, uid) {
  let group = localStorage.getItem("group");
  let d = getUserGroupStr(group);
  let b = { imageId: iid, data: data };
  uid != undefined && (b["userId"] = uid);
  return ajaxPost(`/api/${d}/saveAttrLabel`, b);
}
function getCount() {
...
function getDataBlob(id) {
  ...
  return new Promise((resolve, reject) => {
    ajaxFile("/api/admin/downloadDatas", b)
      .then(data => {
        console.log(data);
        var blob = new Blob([JSON.stringify(data.d, null, 2)], {
